...
class _EthernetLinesDialogState extends State<EthernetLinesDialog> {
  ...
  Future<void> _save(WidgetRef ref) async {
    ...
    try {
      final linesCount = int.tryParse(_linesController.text) ?? 0;
      ...
      if (mounted) {
        debugPrint('EthernetLinesDialog save failed: $e\n$st');
        await ErrorFeedback.show(
          context,
          e,
          fallbackMessage: 'Не удалось сохранить количество линий.',
        );
      }
    } finally {
      if (mounted) {
      ...
      child: Column(
        ...
        children: [
          Text(
            ...
            style: context.appTextStyles.secondaryBody.copyWith(
              color: Theme.of(context).colorScheme.onSurfaceVariant,
              height: 1.3,
            ),
          ),
          const SizedBox(height: 12),
          _buildQuickValues(context, themeColor),
  ...
  Widget _buildQuickValues(BuildContext context, Color themeColor) {
    final textStyles = context.appTextStyles;
      ...
      children: _quickValues.map((value) {
        return SizedBox(
          ...
          child: OutlinedButton(
            ...
            child: Text(
              '$value',
              style: textStyles.bodyStrong.copyWith(color: themeColor),
            ),
          ),
        );
      }).toList(),
