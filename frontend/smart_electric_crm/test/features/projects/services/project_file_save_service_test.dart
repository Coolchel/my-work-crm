...
void main() {
  ...
  test('saves generated bytes to selected desktop path', () async {
    final tempDir = await Directory.systemTemp.createTemp(
      'project-file-save-service-bytes-',
    );
    addTearDown(() => tempDir.delete(recursive: true));

    final targetPath = '${tempDir.path}${Platform.pathSeparator}estimate.pdf';
    ...
    final result = await service.saveBytes(
      bytes: Uint8List.fromList([3, 1, 4, 1, 5]),
      displayName: 'estimate.pdf',
    );

    expect(result.isSaved, isTrue);
    expect(result.path, targetPath);
    expect(await File(targetPath).readAsBytes(), [3, 1, 4, 1, 5]);
  });

  test('uses browser download flow for generated bytes when requested',
      () async {
    Uint8List? savedBytes;
    String? savedFileName;
    ...
    expect(result.isSaved, isTrue);
    expect(result.message, 'Файл передан браузеру для сохранения.');
    expect(savedBytes, [8, 6, 7, 5, 3, 0, 9]);
    expect(savedFileName, 'estimate.pdf');
  });
}
