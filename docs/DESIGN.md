...
### 4.1 Known issue: Settings в нижней навигации
- ID: `NAV-SETTINGS-001`
- Приоритет: `Medium`
- Статус: `known issue / tech debt`
- Описание:
  - Требование (дизайн-контракт): `Settings` не должен находиться в нижней навигации.
  - Фактическая реализация: в текущем `HomeScreen` вкладка `Settings` может добавляться в `NavigationBar` (постоянно при `showWelcome == false` и временно при сценарии открытия из Welcome).
  - Риск: непоследовательная навигационная модель между сценариями входа в Home, ухудшение предсказуемости UX и расхождение с утвержденным дизайн-контрактом.
- Затронутые файлы:
  - `docs/DESIGN.md` (навигационный контракт)
  - `frontend/smart_electric_crm/lib/src/features/home/presentation/screens/home_screen.dart` (текущая реализация)
- Рекомендованное направление исправления:
  - Привести поведение Home к инварианту: исключить `Settings` из `NavigationBar` для всех сценариев, оставить вход в настройки через верхнее действие/явный переход.
